...
function PaymentSuccessHandler({ onSuccess }: { onSuccess: () => void }) {
  ...
  useEffect(() => {
    if (searchParams.get("payment") === "success") onSuccess();
...
export default function ResumePage() {
  ...
  useEffect(() => {
    const calc = () => {
      ...
      setPreviewScale(Math.min(panelW / 794, panelH / 1123, 0.85));
    };
    calc();
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      if (res.status === 401) {
        setStatus({ msg: "Please sign in first.", type: "error" });
        return;
      }
      if (res.status === 402) {
  ...
  return (
    <>
      ...
      {/* Payment handler isolated in Suspense — required by Next.js 15 */}
      <Suspense fallback={null}>
        <PaymentSuccessHandler
          onSuccess={() => {
            setIsPaid(true);
            setStatus({
              msg: "Payment confirmed — download away!",
              type: "success",
            });
          }}
        />
          ...
          >
            ...
            <Section label="Professional Summary">
              <textarea
                ...
                onChange={(e) =>
                  setResumeData((d) => ({ ...d, summary: e.target.value }))
                }
            ...
            {previewHtml ? (
              ...
              <div style={{ textAlign: "center", color: "#94a3b8" }}>
                ...
                <p style={{ fontSize: "14px" }}>
                  Start by entering your name above
                </p>
              </div>
            )}
          </div>
